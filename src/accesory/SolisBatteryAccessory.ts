...
export class SolisBatteryAccessory {
  ...
  constructor(
    ...
    if (batterySensor) {
      ...
      this.batterySensorService.setCharacteristic(
        this.platform.Characteristic.StatusLowBattery, this.platform.Characteristic.StatusLowBattery.BATTERY_LEVEL_NORMAL
      );
    }
  ...
  async pollBatteryStatus() {
    const { keyId, keySecret, stationId, batterySensor, solarSensor, netSensor, loadSensor, multiplier } = this.getConfigData();
    ...
    if (batteryData) {
      ...
      if (batterySensor) {
        ...
        if (batteryData.data.batteryChargeEnergy > 0) {
          this.batterySensorService?.updateCharacteristic(
            this.platform.Characteristic.ChargingState, this.platform.Characteristic.ChargingState.CHARGING
          );
        } else {
          this.batterySensorService?.updateCharacteristic(
            this.platform.Characteristic.ChargingState, this.platform.Characteristic.ChargingState.NOT_CHARGING
          );
        }

        if (batteryData.data.batteryPower < 0) {
          this.batterySensorService?.updateCharacteristic(
            this.platform.Characteristic.Name, `Battery: ${batteryData.data.batteryPower}${batteryData.data.batteryPowerStr}`,
          );
        } else {
      ...
      if (solarSensor) {
        ...
        const value = this.convertPowerNumber(batteryData.data.power) * multiplier;
        this.solarSensorService?.updateCharacteristic(this.platform.Characteristic.CurrentAmbientLightLevel, value);
        this.solarSensorService?.updateCharacteristic(
          this.platform.Characteristic.Name, `Solar: ${(batteryData.data.porwerPercent*100).toFixed(2)}%`,
        );
      }

      if (netSensor) {
        this.platform.log.debug(`Setting net usage information`);
        const value = this.convertPowerNumber(batteryData.data.psum) * multiplier;
        this.netSensorService?.updateCharacteristic(this.platform.Characteristic.CurrentAmbientLightLevel, value);
        this.netSensorService?.updateCharacteristic(this.platform.Characteristic.StatusActive, batteryData.data.psum > 0);
        this.netSensorService?.updateCharacteristic(
          this.platform.Characteristic.Name, `Net: ${batteryData.data.psum.toFixed(3)}${batteryData.data.psumStr}`
        );
      }
      ...
      if (loadSensor) {
        ...
        const value = this.convertPowerNumber(totalLoad) * multiplier;
        this.loadSensorService?.updateCharacteristic(this.platform.Characteristic.CurrentAmbientLightLevel, value);
        this.loadSensorService?.updateCharacteristic(
          this.platform.Characteristic.Name, `Usage: ${totalLoad.toFixed(3)}${batteryData.data.psumStr}`
        );
      }
    } else {
  ...
  getConfigData() {
    const { keyId, keySecret, stationId, batterySensor, solarSensor, netSensor, loadSensor, multiplier } = this.platform.config;
    return { keyId, keySecret, stationId, batterySensor, solarSensor, netSensor, loadSensor, multiplier };
  }
